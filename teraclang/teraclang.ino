...
//---------------variable zone---------------
  ...
  double arrowUpDown = joy_orig.axes[7];
  double arrowLeftRight = joy_orig.axes[6];
  bool logitech = joy_orig.buttons[8];
...
//---------------move zone---------------
  ...
  //move backward
  else if(joyX < 0.3 && joyZ == 0){
    speed = abs(joyX * 1023);
    moveBackward(speed);
    
  }
  //turn left
  else if(joyX == 0 && joyZ > 0.3){
    speed = joyZ * 1023;
    moveLeft(speed);
  }
  //turn right
